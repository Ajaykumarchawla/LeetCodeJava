...
public class PacificAtlanticWaterFlow {
    public void findFlow(int i, int j, int[][] ocean, int[][] height, int digit) {
        if (i >= 0 && i < height.length && j >= 0 && j < height[0].length) {
            if (ocean[i][j] == 0 && digit <= height[i][j]) {
                ocean[i][j] = 1;
                findFlow(i - 1, j, ocean, height, height[i][j]);
                findFlow(i + 1, j, ocean, height, height[i][j]);

                findFlow(i, j - 1, ocean, height, height[i][j]);
                findFlow(i, j + 1, ocean, height, height[i][j]);
    ...
    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        ...
        for (int i = 0; i < heights.length; i++) {
            for (int j = 0; j < heights[0].length; j++) {
                if ((i == 0 || j == 0) && pacific[i][j] == 0) {
                    findFlow(i, j, pacific, heights, -1);
                }

                if (i == heights.length - 1 || j == heights[0].length - 1 && atlantic[i][j] == 0) {
                    findFlow(i, j, atlantic, heights, -1);
