...
public class WordLadderII {
    private List<List<String>> getFinalList(Map<String, List<String>> map, String beginWord, String word) {
        if (word == beginWord) {
            ...
            List<String> pet = map.get(word);

            if (pet == null) {
                return returnList;
            }
            for (String str : pet) {
                List<List<String>> currentList = getFinalList(map, beginWord, str);
                for (List<String> current : currentList) {
                    current.add(word);
    ...
    public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
        ...
        visited.put(beginWord, firstlist);
        counter.put(beginWord, 0);
        ...
        while (!queue.isEmpty()) {
            ...
            if (count >= counter.getOrDefault(endWord, Integer.MAX_VALUE)) {
                break;
            }
            ...
            for (int i = 0; i < word.length(); i++) {
                ...
                // System.out.println(key+" "+hits);
                for (String hit : hits) {
                    if (hit == word || hit == beginWord) {
                        continue;
                    }
                    if (visited.containsKey(hit)) {

                        int currentCount = counter.get(hit);
                        if (currentCount == count + 1) {
                            visited.get(hit).add(word);
                            // counter.put(hit,count+1);
                        }
                    } else {
                        List<String> newList = new LinkedList<>();
                        newList.add(word);
                        visited.put(hit, newList);
                        counter.put(hit, count + 1);
                        queue.add(hit);
                    }
                }
        ...
        return getFinalList(visited, beginWord, endWord);
